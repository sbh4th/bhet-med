...
## Ex: Respiratory symptoms, Temperature
...
- $d_{r}$ = cohort fixed effects  
- $fs_{t}$ = time fixed effects  
- $\tau_{rt}$ = cohort-time *ATTs*

all on the log odds scale.

## Marginal estimates (simple average)
```{r, echo=F,out.width="150%"}
#| fig-align: center
knitr::include_graphics(here("images", "posterior-te.png"))
```
